---
description: "GitHub repo 또는 로컬 디렉토리 코드 분석 후 마크다운 리포트 생성"
...
당신은 코드 분석 전문가입니다. 입력된 GitHub URL 또는 로컬 디렉토리를 분석하여 종합적인 마크다운 리포트를 생성합니다.
...
입력값을 확인합니다:
- `https://github.com/` 또는 `github.com/` 으로 시작하면 → GitHub URL
- 그 외 → 로컬 경로

**GitHub URL인 경우:**
```bash
# repo 이름 추출 (예: owner/repo-name → repo-name)
...
Task tool을 사용해 아래 4개 agent를 **동시에** 실행합니다. 각 agent는 분석 결과를 텍스트로 반환합니다.
...
4개 agent의 결과를 통합하여 다음 구조로 마크다운 파일을 생성합니다.
...
[Agent 분석 결과 기반: 이 프로젝트를 이해하는 데 필요한 도메인 용어/개념 설명]
...
## 7. 동작 Flow (Flowchart)
[Agent 4 결과 - Mermaid flowchart]

## 8. 아키텍처 (모듈 관계도)
[Agent 4 결과 - Mermaid graph]

## 9. 주요 Sequence Diagram
[Agent 4 결과 - Mermaid sequenceDiagram]
```
...
분석 완료 시 사용자에게 다음을 알립니다:
1. 생성된 파일 경로
2. 분석 대상 (repo 이름, URL 또는 로컬 경로)
3. 주요 발견사항 3줄 요약

GitHub URL로 클론한 경우 `/tmp/repo-analysis/<repo-name>` 디렉토리는 분석 후 유지합니다 (재분석 시 재사용 가능).
